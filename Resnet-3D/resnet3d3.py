...
Co-Learning Feature Fusion Maps from PET-CT
Images of Lung Cancer
Localisation/Segmentation no longer use any of the initial downsampling layers
"""
...
class Resnet3DBuilder(object):
    ...
    def get_colearning_block(encoder_outputs, reg_factor, index):
        ...
        fusion_map = conv4d_wrapper(num_filters=stacked_features._keras_shape[CHANNEL_AXIS], 
                                         kernel_size=(2,3,3,3),  #experimental conv4d only operates on a channels-first basis
    ...
    def build_decoder(colearn_blocks, block_fn, repetitions, reg_factor, mode):
        if mode == 'detection' or mode == 'localisation':
            ...
            #reduce number of filters from 1024 to 128 so that the dense layer isn't too tired

            return _bn_relu(colearn_blocks[0])
    ...
    def build(input_shape, output_shape, block_fn, repetitions, reg_factor, mode='detection'):
        ...
        if mode == 'detection':
            ...
            final_output = concatenate([flatten, input[1]]) #predicted centre is fed in here
            
            #centre prediction
            dense_centre = Dense(units=output_shape[0],
                          kernel_initializer="he_normal",
                          activation='linear',
                          kernel_regularizer=l2(reg_factor))(final_output)
            
            dense_centre = Add()([dense_centre, input[1]])
            ...
            dense_centre = expand_dims(axis=1)(dense_centre)
            dense_size = expand_dims(axis=1)(dense_size)
